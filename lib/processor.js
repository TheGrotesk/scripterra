...
module.exports = class Processor {
  ...
  async _processSchedule(name, env, expression) {
    ...
    await this.scheduler.scheduleV2(name, env, expression);
  }
  ...
  async _processListAll() {
    ...
    const list = await scriptEntity.search().return.all();

    const scheduleEntity = await this.database.getEntity('Schedules');

    const schedulesList = await scheduleEntity.search().return.all();
