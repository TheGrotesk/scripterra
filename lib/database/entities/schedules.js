module.exports = class Schedules {
  ...
  async delete(script, env) {
    if (env) {
      return this.db.run(`DELETE FROM ${this.name} WHERE script='${script}' AND env='${env}'`);
    } else {
      return this.db.run(`DELETE FROM ${this.name} WHERE script='${script}'`);
    }
  }
